...
export function hasEbayBrowseCredentials() {
  ...
  return Boolean(process.env.EBAY_ACCESS_TOKEN || (config.clientId && config.clientSecret));
}
...
export async function getApplicationAccessToken({ forceRefresh = false } = {}) {
  const suppliedToken = String(process.env.EBAY_ACCESS_TOKEN || '').trim();
...
export async function enrichProductsWithBrowseImages(
  products,
  { maxItems = Infinity, force = false, exactOnly = true } = {},
) {
  ...
  for (const product of products) {
    ...
    if (directImages.length) {
      ...
      product.imageStatus = 'exact-ebay-item-match';
      directMatches += 1;
      results.push({
        id: product.id,
    ...
    if (existingImages.length || exactOnly) {
      preserved += 1;
      continue;
    }
