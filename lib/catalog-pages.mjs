...
const PRODUCT_PAGE_ALIASES = new Map([
  [
    'chevrolet-tahoe-5g-gmc-1yc-2020-2026-performance-big-brake-kit-front-8-pot-405mm',
    '126367368319',
  ],
]);
...
function mergeDuplicateProduct(kept, candidate) {
  ...
  return {
    ...kept,
    ebayItemId: kept.ebayItemId || candidate.ebayItemId || '',
    images,
    tags,
...
async function rewriteProductAliasPages(products) {
  const byHandle = new Map(products.map((product) => [product.handle, product]));
  await Promise.all([...PRODUCT_PAGE_ALIASES.entries()].flatMap(([alias, targetHandle]) => {
    const target = byHandle.get(targetHandle);
    if (!target) return [];
    const aliasProduct = { ...target, handle: alias };
    const filePath = path.join(productsDir, `${alias}.html`);
    return [writeFile(filePath, buildProductPage(aliasProduct, products), 'utf8')];
  }));
}
...
async function rewriteCollectionPages(products) {
  ...
  // Keep the storefront's familiar shortcuts while sourcing every card from
  // the current US catalog. Old localized collection files otherwise leave
  // links to listings that were intentionally removed from PATZCOM.
  const compatibilityPages = [
    {
      label: 'Big Brake Kits',
      slug: 'big-brake-kit',
      products: products.filter((product) => categoryMeta(product.type).slug === 'brake-calipers-brackets'),
    },
    {
      label: 'Heavy-Duty Durability',
      slug: 'heavy-duty-durability',
      products: products.filter((product) => /lift|spring|shock|strut|sway/i.test(`${product.title} ${product.type}`)),
    },
    {
      label: 'Rear Sway Bar',
      slug: 'rear-anti-roll-bar',
      products: products.filter((product) => categoryMeta(product.type).slug === 'sway-bars-links-bushings'),
    },
    {
      label: 'Strut Tower Bars',
      slug: 'strut-tower-bar-under-bars',
      products: products.filter((product) => /strut tower|under bar|chassis brace/i.test(`${product.title} ${product.type}`)),
    },
  ];
  const pagesBySlug = new Map(pages.map((page) => [page.slug, page]));
  for (const page of compatibilityPages) {
    if (page.products.length && !pagesBySlug.has(page.slug)) {
      pages.push(page);
  ...
  await removeStaleHtmlFiles(
    path.join(siteRoot, 'collections'),
    new Set(pages.map((page) => `${page.slug}.html`)),
  );
}
...
function normalizeSingleProduct(item, index, usedHandles) {
  ...
  return {
    ...
    listingSite: String(item.listingSite || item.site || '').trim(),
    ebaySource: String(item.ebaySource || item.source || '').trim(),
...
export async function saveCatalogSnapshot(products, { deduplicate = true } = {}) {
  const dedupedProducts = deduplicate ? dedupeCatalogProducts(products) : products;
  ...
  await rewriteProductAliasPages(dedupedProducts);
  await removeStaleHtmlFiles(
    productsDir,
    new Set([
      ...dedupedProducts.map((product) => `${product.handle}.html`),
      ...[...PRODUCT_PAGE_ALIASES.keys()].map((alias) => `${alias}.html`),
    ]),
  );
