...
import { hasEbayBrowseCredentials } from './ebay-browse.mjs';
...
export async function getIntegrationStatus() {
  ...
  const ebayBrowseReady = hasEbayBrowseCredentials();
  const paypalReady = Boolean(process.env.PAYPAL_CLIENT_ID);
  const paypalServerReady = Boolean(process.env.PAYPAL_CLIENT_ID && process.env.PAYPAL_CLIENT_SECRET);
  const stripeReady = Boolean(process.env.STRIPE_SECRET_KEY);

  return {
    ebay: {
      ready: ebayKeysReady,
      browseApiReady: ebayBrowseReady,
      ...
      apiReadiness: {
        ...
        imageEnrichment: ebayBrowseReady ? 'ready-for-item-image-sync' : 'needs-app-id-and-cert-id',
    ...
    paypal: {
      ready: paypalReady,
      serverReady: paypalServerReady,
      environment: process.env.PAYPAL_ENV || 'live',
      onboardingStatus: paypalServerReady ? 'server-checkout-ready' : paypalReady ? 'client-id-only' : 'awaiting-live-credentials',
    },
    stripe: {
      ready: stripeReady,
      onboardingStatus: stripeReady ? 'checkout-ready' : 'awaiting-secret-key',
    },
  };
}
